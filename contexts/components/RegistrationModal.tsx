...
const RegistrationModal: React.FC<RegistrationModalProps> = ({ course, onClose }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const errorMessage = err?.message || 'เกิดข้อผิดพลาดในการลงทะเบียน กรุณาลองใหม่อีกครั้ง';
  ...
  const renderSuccessView = () => (
    <div className="p-6 sm:p-8 text-center flex flex-col items-center">
      ...
      <div className="mt-6 p-4 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg w-full max-w-md">
        <p className="text-sm text-green-600 dark:text-green-400 font-medium uppercase tracking-wide">รหัสการลงทะเบียนของคุณ</p>
        <p className="text-3xl font-bold text-green-700 dark:text-green-300 tracking-wider mt-1">{successfulRegistration?.registrationId}</p>
      </div>

      {/* Course Summary */}
      <div className="mt-6 w-full max-w-md bg-gray-50 dark:bg-gray-700/50 rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden text-left shadow-sm">
        ...
        <div className="p-4 space-y-3">
            <div>
               <p className="text-xs text-gray-500 dark:text-gray-400 uppercase font-semibold">หลักสูตร</p>
               <p className="font-medium text-gray-800 dark:text-gray-100">{course.courseName}</p>
            </div>
             <div className="grid grid-cols-2 gap-4">
                <div>
                   <p className="text-xs text-gray-500 dark:text-gray-400 uppercase font-semibold">รุ่นที่</p>
                   <p className="font-medium text-gray-800 dark:text-gray-100">{course.courseGen}</p>
                </div>
                 <div>
                   <p className="text-xs text-gray-500 dark:text-gray-400 uppercase font-semibold">สถานที่</p>
                   <p className="font-medium text-gray-800 dark:text-gray-100 truncate" title={course.location}>{course.location}</p>
                </div>
            </div>
             <div>
               <p className="text-xs text-gray-500 dark:text-gray-400 uppercase font-semibold">วันที่อบรม</p>
               <p className="font-medium text-gray-800 dark:text-gray-100">{formatDate(course.startDate)} - {formatDate(course.endDate)}</p>
            </div>
        </div>
      </div>

      <p className="mt-6 text-sm text-gray-500 dark:text-gray-400 max-w-md">
        ระบบได้ส่งอีเมลยืนยันพร้อมรายละเอียดการลงทะเบียนไปยัง <span className="font-medium text-gray-700 dark:text-gray-300">{formData.email}</span> แล้ว
      </p>
