...
const Sidebar: React.FC<SidebarProps> = ({ activePage, setActivePage, isOpen, setIsOpen, activeAdminView, setActiveAdminView }) => {
  const { user } = useAuth();
      ...
      <aside className={`fixed top-0 left-0 h-full bg-gray-800 dark:bg-gray-900 text-white w-64 space-y-6 py-7 px-2 transform transition-transform duration-300 z-30 ${isOpen ? 'translate-x-0' : '-translate-x-full'} md:relative md:translate-x-0 md:h-auto flex flex-col`}>
        <div>
            <div className="flex items-center space-x-2 px-4">
              ...
              <span className="text-2xl font-extrabold">OCRS.</span>
            </div>
        </div>
           ...
           {user && (
            <>
                <hr className="my-4 border-gray-600" />
                 <p className="px-4 text-xs font-semibold text-gray-400 uppercase tracking-wider mb-2">Admin Menu</p>
                 <ul className="space-y-2">
                    <NavItem
                        icon={<AdminIcon />}
                        label="Dashboard"
                    ...
                    />
                    {/* System settings moved to Header User Menu */}
                </ul>
            </>
           )}
        </nav>
